//querySelector() function gets the corresponding CSS selector
...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  setInterval(getcurrTime, 1000); // calls getcurrTime() for every sec
  fetchAlarm(); 
});
...
function getcurrTime() {
  ...
  time = time.toLocaleTimeString("en-US", {
    ...
    minute: "numeric",
    second: "numeric",
    hour12: true, // setting 12 hr format
  });
...
//function to ring the alarm when alarm time matches with current time
function setAlarm(time, fetching = false) {
  const alarm = setInterval(() => {
    if (time === getcurrTime()) {
      alert('Time to ring your Alarm !!')
    }
  }, 500);
...
//function used to delete the alarm when user click on delete button
